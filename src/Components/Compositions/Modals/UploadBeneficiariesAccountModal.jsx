...
import { uploadBeneficiaryAccountAction } from '../../../redux/actions/upload-beneficiary-account.action';
...
const mapStateToProps = ({uploadBeneficiaryAccount}) => ({
  uploadingAccount: uploadBeneficiaryAccount.uploadingAccount
})

const UploadBeneficiariesAccountModal = ({ toggleDropdown, prog_id }) => {
  ...
  const uploadBeneficiaryAccount = useCallback((formData, config={}) => uploadBeneficiaryAccountAction(formData, config), [dispatch])


  const {uploadingAccount} = useSelector(mapStateToProps)
  ...
  const handleUpload = async () => {
    ...
    if (myFiles.length) {
      formData.append('ben', myFiles[0], myFiles[0]?.name);
      formData.append('programme_id', prog);

      try {
          
          // const res = await uploadBeneficiaryAccount(formData, {
          const res = await apiCall('beneficiary-acc-upload', 'post', formData, {            
  ...
  return (
    ...
    <ModalWrapper onClick={closeModal} id="modalRefWrapper" >
      ...
      <CancelIcon src="/cancel-icon.svg" alt="cancel" onClick={toggleDropdown } />
        <Modal.Header>Upload Beneficiaries Account</Modal.Header>
        <UploadWrapper>
          <DropzoneWrapper>
        ...
        <Modal.Buttons>
          <Button onClick={() => {
            ...
            {uploadingAccount ? 'loading' : 'Upload'}
          </Button>
...
export default UploadBeneficiariesAccountModal;